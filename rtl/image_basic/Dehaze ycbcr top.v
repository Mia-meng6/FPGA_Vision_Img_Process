...
// -----------------------------------------------------------------------------
// 子模块：CAP_Y_Dehaze（对 Y 通道做 CAP 去雾）
// -----------------------------------------------------------------------------
module CAP_Y_Dehaze #(
...
wire vsync_fall = vsync_d1 & ~i_vsync;  // vsync 下降沿：帧结束时刻

// A_y 低通滤波
// A_y_new = A_y_old × 7/8 + A_raw × 1/8
// 硬件：A_y + ((A_raw - A_y) >> 3)
...
d_inv_t_rom u_d_inv_y (
    .clock   (clk),
    .address ({1'b0, d_index}),  // 9-bit地址：高位补0，访问范围0~255
    .data    (16'd0),            // ROM只读，写数据恒为0
    .wren    (1'b0),             // ROM只读，写使能禁止
    .q       (inv_t_rom_data)
);
...
// -----------------------------------------------------------------------------
// 顶层：Dehaze_YCbCr_Top (Retinex 旁路调试版)
// -----------------------------------------------------------------------------
